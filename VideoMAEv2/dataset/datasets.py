...
import warnings
import sys
sys.path.insert(0, '/home/vislab-001/Jared/Envy_AI_City')
...
class VideoClsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        elif self.mode == 'test' and self.test_num_crop == 1 and self.test_num_segment == 1:
            sample = self.dataset_samples[index]
            buffer = self.load_video(sample)
            if len(buffer) == 0:
                while len(buffer) == 0:
...
class RawFrameClsDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.dataset_samples = list(
            cleaned[0].apply(lambda row: os.path.join(self.data_root, row)))
        self.total_frames = list(map(lambda x: len(os.listdir(x)), self.dataset_samples))
        self.label_array = list(cleaned.values[:, -1])
    ...
    def load_frame(self, sample, num_frames, sample_rate_scale=1):
        ...
        try:
            all_frame_paths = list(map(lambda x: fname + '/' + x, os.listdir(fname)))
        except Exception as e:
            print(f"Failed to load frames from {fname} with error {e}!")
            return []

        length = len(all_frame_paths)
        ...
        if self.mode == 'test':
            ...
            sampled_frames = [all_frame_paths[i] for i in all_index]
            buffer = np.array(list((map(self.image_loader, sampled_frames))))
            return buffer
