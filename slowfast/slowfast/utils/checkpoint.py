...
def load_test_checkpoint(cfg, model, ckpt=None):
    """
    Loading checkpoint logic for testing.
    """
    if ckpt is not None:
        if cfg.MODEL.MODEL_NAME == "VideoMAEv2":
            load_videomaev2_checkpoint(cfg, model, ckpt)
        else:
            load_checkpoint(ckpt, model, cfg.NUM_GPUS > 1)
...
def load_videomaev2_checkpoint(cfg, model, ckpt=None):
    tubelet_size = 2
    ...
    logger.info("Load ckpt from %s" % ckpt)
    ...
    for k in ['head.weight', 'head.bias']:
        if k in checkpoint_model and checkpoint_model[
            ...
            if checkpoint_model[k].shape[
                    0] == 710 and cfg.TRAIN.DATASET.startswith('Kinetics'):
                logger.info(f'Convert K710 head to {cfg.TRAIN.DATASET} head')
                if cfg.TRAIN.DATASET == 'Kinetics-400':
                    label_map_path = 'misc/label_710to400.json'
                elif cfg.TRAIN.DATASET == 'Kinetics-600':
                    label_map_path = 'misc/label_710to600.json'
                elif cfg.TRAIN.DATASET == 'Kinetics-700':
    ...
    if 'pos_embed' in checkpoint_model:
        ...
        orig_size = int(
            ((pos_embed_checkpoint.shape[-2] - num_extra_tokens) //
                (cfg.DATA.NUM_FRAMES // model.patch_embed.tubelet_size))**0.5)
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.reshape(
                -1, cfg.DATA.NUM_FRAMES // model.patch_embed.tubelet_size,
    ...
    elif cfg.DATA.TEST_CROP_SIZE != 224:
        pos_tokens = model.pos_embed
        org_num_frames = 16
        T = org_num_frames // tubelet_size # tublet size
        P = int((pos_tokens.shape[1] // T)**0.5)
        C = pos_tokens.shape[2]
        new_P = cfg.DATA.TEST_CROP_SIZE // patch_size[0]
        # B, L, C -> BT, H, W, C -> BT, C, H, W
    ...
    if cfg.DATA.NUM_FRAMES != 16:
        ...
        pos_tokens = model.pos_embed
        new_T = cfg.DATA.NUM_FRAMES // tubelet_size
        P = int((pos_tokens.shape[1] // T)**0.5)
        C = pos_tokens.shape[2]
        pos_tokens = pos_tokens.reshape(-1, T, P, P, C)
    ...
    VideoMAEv2_Utils.load_state_dict(
        model, checkpoint_model, prefix='')
