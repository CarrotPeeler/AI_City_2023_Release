...
if __name__ == '__main__':  
    ...
    inc_dir = os.getcwd() + f"/evaluation/val_preds/{model_name}/incorrect_preds/"
    cor_dir = os.getcwd() + f"/evaluation/val_preds/{model_name}/correct_preds/"
    save_dir = os.getcwd() + f"/evaluation/val_preds/{model_name}/checkpoint_stats"

    if not os.path.exists(save_dir):
        os.mkdir(save_dir)

    csv_filepaths = [ #[f"val_incorrect_pred_probs_{model_name}_120_epochs.txt", f"val_correct_pred_probs_{model_name}_120_epochs.txt"],
                    #  [f"val_incorrect_pred_probs_{model_name}_135_epochs.txt", f"val_correct_pred_probs_{model_name}_135_epochs.txt"],
                     [f"val_incorrect_pred_probs_{model_name}_160_epochs.txt", f"val_correct_pred_probs_{model_name}_160_epochs.txt"],
                     [f"val_incorrect_pred_probs_{model_name}_180_epochs.txt", f"val_correct_pred_probs_{model_name}_180_epochs.txt"],
                     [f"val_incorrect_pred_probs_{model_name}_200_epochs.txt", f"val_correct_pred_probs_{model_name}_200_epochs.txt"]
                    ]
